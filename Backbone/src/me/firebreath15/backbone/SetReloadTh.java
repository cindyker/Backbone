...
public class SetReloadTh implements Listener{
	...
	public void onClick(PlayerInteractEvent event){
		...
		if(event.getAction() == Action.RIGHT_CLICK_BLOCK){
			...
			plugin.getConfig().set("reloader3.x",reloadx);
			plugin.getConfig().set("reloader3.y",reloady);
			plugin.getConfig().set("reloader3.z",reloadz);
			plugin.getConfig().set("reloader3.world",event.getPlayer().getWorld().getName());
			plugin.saveConfig();
			event.getPlayer().sendMessage(ChatColor.GREEN+"You set the reloading station for arena 3!");
